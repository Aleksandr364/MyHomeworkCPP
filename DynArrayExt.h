...
template<typename T> class DynArrayExtIterator
{

};

template<typename T> class DynArrayExt
{
...
protected:
	...
	void CopyArray(const DynArrayExt& rhs)
	{
	...
	void MoveArray(DynArrayExt&& rhs) noexcept
	{
...
public:
	...
	DynArrayExt(uint32_t max_size, uint32_t step) : max_size(max_size), step(step)
	{
		Allocate();
		std::cout << "DynArrayExt param. constructor.\n";
	...
	{
		DestructData();
		std::cout << "DynArrayExt destructor.\n";
		std::cout << std::hex << std::showbase << this << std::noshowbase << std::dec << "\n";
	}
};
//	bool Empty()
